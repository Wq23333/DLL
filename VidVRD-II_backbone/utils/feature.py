...
def extract_object_feature_predcls(dname, vid, fstart, fend, anno, include_gt=False, verbose=False):
    vsig = common.get_segment_signature(vid, fstart, fend)
    path = common.get_feature_path(dname, 'object', anno.get('video_path', vid).replace('.mp4', ''))
    path = os.path.join(path, '{}-{}.h5'.format(vsig, 'object'))
    if os.path.exists(path):
    ...
    tracklets = tracklets[num_det:]
    track_gt_id = track_gt_id[num_det:]
    pairwise_iou = pairwise_iou[num_det:, num_det:]
    
    index_exist = []
    for i in range(len(tracklets)):
        if (len(tracklets[i]["feature"])>0 and len(tracklets[i]["bboxes"])>0 ):
    ...
    if (index_exist==[]):
        ...
        tracklets_ = []
        track_gt_id_ = []
        pairwise_iou_ = np.empty((len(index_exist), len(index_exist)), dtype=np.float32)
        for i in range(len(index_exist)):
            tracklets_.append(tracklets[index_exist[i]])
            track_gt_id_.append(track_gt_id[index_exist[i]])
            for j in range(len(index_exist)):
                pairwise_iou_[i][j] = (pairwise_iou[index_exist[i]][index_exist[j]])

        return tracklets_, track_gt_id_, pairwise_iou_
...
def extract_relation_feature(dname, vid, fstart, fend, anno, sampled_pair_ids=None, include_gt=False, verbose=False):
    ...
    for ti, tj in pairs:
        ...
        sx,sy,sw,sh = _convert_bbox(subj['bboxes'][int(len(subj['bboxes'])/2)])
        ox,oy,ow,oh =  _convert_bbox(obj['bboxes'][int(len(obj['bboxes'])/2)])
        sub_middle = np.asarray([sx,sy,sw,sh], dtype=np.float32)
        obj_middle = np.asarray([ox,oy,ow,oh], dtype=np.float32)
        middle_feat = np.concatenate([sub_middle, obj_middle])
        middle_feats.append(middle_feat.astype(np.float32))

        sx,sy,sw,sh = _convert_bbox(subj['bboxes'][0])
        ox,oy,ow,oh =  _convert_bbox(obj['bboxes'][0])
        sub_start = np.asarray([sx,sy,sw,sh], dtype=np.float32)
        obj_start= np.asarray([ox,oy,ow,oh], dtype=np.float32)
        start_feat = np.concatenate([sub_start, obj_start])
        start_feats.append(start_feat.astype(np.float32))
    ...
    middle_feats = np.asarray(middle_feats)
    start_feats = np.asarray(start_feats)
    ...
    if not include_gt:
        ...
        middle_feats = middle_feats[pair_ids]
        start_feats = start_feats[pair_ids]
        pairwise_iou = pairwise_iou[:num_det, :]

    return pairs, sub_feats, obj_feats, pred_pos_feats, pred_vis_feats, pairwise_iou, track_gt_id, middle_feats, start_feats
def extract_relation_feature_predcls(dname, vid, fstart, fend, anno, sampled_pair_ids=None, include_gt=False, verbose=False):
    tracklets, track_gt_id, pairwise_iou = extract_object_feature_predcls(dname, vid, fstart, fend, anno, include_gt=include_gt, verbose=verbose)
